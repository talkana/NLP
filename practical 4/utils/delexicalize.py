        ...
        for ent in db_json:
            for key, val in list(ent.items()):
                ...
                # YOUR CODE HERE:
                    ...
                    delex_area.append((normalize(val), f'[value_{Slots.AREA.value}]'))
                elif key == Slots.FOOD.value:
                    delex_food.append((normalize(val), f'[value_{Slots.FOOD.value}]'))
                elif key == Slots.PRICERANGE.value:
                    delex_price.append((normalize(val), f'[value_{Slots.PRICERANGE.value}]'))
                # YOUR CODE ENDS HERE.
                else:
                    pass
