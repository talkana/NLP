...
def load_model_and_data(num):
    ...
    model = Model(
        args, input_lang_index2word,
        output_lang_index2word, input_lang_word2index,
...
def decode(num=1):
    model, val_dials, test_dials = load_model_and_data(num)
    ...
    for ii in range(2):
        ...
        with open(
            args.valid_output + 'val_dials_gen_' + str(num) + '.json', 'w'
        ) as outfile:
        ...
        with open(
            args.decode_output + 'test_dials_gen_' + str(num) + '.json', 'w'
        ) as outfile:
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '--no_models', type=int, default=10, help='how many models to evaluate')
    parser.add_argument(
    ...
    parser.add_argument('--write_n_best', type=util.str2bool, nargs='?',
                        const=True, default=False,
                        help='Write n-best list (n=beam_width)')

    parser.add_argument(
        '--model_path', type=str, default='model/model/model.ckpt',
        help='Path to a specific model checkpoint.')
    parser.add_argument('--model_dir', type=str, default='model/')
    parser.add_argument('--model_name', type=str, default='model.ckpt')

    parser.add_argument(
        '--valid_output', type=str, default='model/data/val_dials/',
        help='Validation Decoding output dir path')
    parser.add_argument(
        '--decode_output', type=str, default='model/data/test_dials/',
