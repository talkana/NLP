...
def naive_softmax_loss_and_gradient(
    ...
    p_o_c = softmax(outsideVectors @ centerWordVec)
    loss = -np.log(p_o_c[outsideWordIdx])
    gradCenterVec = (p_o_c - np.eye(len(p_o_c))[outsideWordIdx]) @ outsideVectors
    gradOutsideVecs = np.outer(p_o_c - np.eye(len(p_o_c))[outsideWordIdx], centerWordVec)
...
def neg_sampling_loss_and_gradient(
    ...
    def sigmoids(x):
        return sigmoid(x @ centerWordVec) - 1

    gradOutsideVecs, gradCenterVec, loss = np.zeros(outsideVectors.shape), np.zeros(centerWordVec.shape), 0
    for i in indices:
        if i == outsideWordIdx:
            gradCenterVec += sigmoids(outsideVectors[i]) * outsideVectors[i]
            gradOutsideVecs[i] += sigmoids(outsideVectors[outsideWordIdx]) * centerWordVec
            loss -= np.log(sigmoid(outsideVectors[i] @ centerWordVec))
        else:
            gradCenterVec -= sigmoids(-outsideVectors[i]) * outsideVectors[i]
            gradOutsideVecs[i] -= sigmoids(-outsideVectors[i]) * centerWordVec
            loss -= np.log(sigmoid(-outsideVectors[i] @ centerWordVec))
    ### END YOUR CODE
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    ### YOUR CODE HERE
    center_i = word2Ind[currentCenterWord]
    center_word_vec = centerWordVectors[center_i]

    for outside_word in outsideWords:
        outside_i = word2Ind[outside_word]
        curr_loss, grad_center, grad_out = word2vecLossAndGradient(center_word_vec, outside_i, outsideVectors, dataset)
        loss += curr_loss
        gradCenterVecs[center_i] += grad_center
