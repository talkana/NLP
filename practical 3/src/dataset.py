...
class CharCorruptionDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        truncated_doc_length = random.randint(4, int(self.block_size * 7 / 8))
        truncated_doc = document[:truncated_doc_length]
        mask_length = random.randint(np.round(1/8 * len(truncated_doc)), np.round(3/8 * len(truncated_doc)))
        first_part_length = random.randint(0, len(truncated_doc) - mask_length)
        first_part, mask, second_part = truncated_doc[:first_part_length], truncated_doc[first_part_length:first_part_length + mask_length], truncated_doc[first_part_length + mask_length:]
        rearranged_doc = first_part + self.MASK_CHAR + second_part + self.MASK_CHAR + mask
        rearranged_doc += self.PAD_CHAR * (self.block_size - len(rearranged_doc))
        x, y = rearranged_doc[:-1], rearranged_doc[1:]
